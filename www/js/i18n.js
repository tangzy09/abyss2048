...
// Locale JSON files live in /locales/<lang>.json
// ════════════════════════════════════════
const I18N = (() => {
  const SUPPORTED = ['zh-CN', 'en', 'es', 'hi', 'bn', 'pt-BR', 'ru', 'ja', 'pa', 'de'];
  ...
  function detect() {
    let saved = '';
    try { saved = Platform.storage.get('dm_lang') || ''; } catch (e) {}
  ...
  async function setLang(l) {
    ...
    dict = await load(l);
    lang = l;
    try { Platform.storage.set('dm_lang', l); } catch (e) {}

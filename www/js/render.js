...
function drawInfoOverlay(){
  const SW=GameGlobal.SW,SH=GameGlobal.SH;fillScreen(!C.dark?'rgba(224,241,251,0.97)':'rgba(0,0,0,0.92)');
...
function drawBackdrop(){
  ...
  if(_scn){const sc=Math.max(FW/_scn.width,FH/_scn.height),dw=_scn.width*sc,dh=_scn.height*sc;ctx.drawImage(_scn,(FW-dw)/2,(FH-dh)/2,dw,dh);
    ...
    ctx.fillStyle=!C.dark?'rgba(230,244,255,0.28)':'rgba(0,10,20,0.32)';ctx.fillRect(0,0,FW,FH);}
}

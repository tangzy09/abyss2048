...
function drawEnergyRow(L){const{PAD,SW,energyY:y,energyH:h}=L;const pct=Math.min(1,(G.energyScore||0)/1000);const trackW=SW-PAD*2-94;txtL(T('ui.energy'),PAD,y+h/2,C.muted,'11px sans-serif');fillRR(PAD+46,y+h/2-3,trackW,6,3,C.border);if(pct>0)fillRR(PAD+46,y+h/2-3,trackW*pct,6,3,C.accent);txtR(`${Math.floor(G.energyScore||0)}/1000`,SW-PAD,y+h/2,C.muted,'10px sans-serif');}
...
function renderAll(){
  if(!ctx)return;hitAreas=[];const L=layout();const SW=GameGlobal.SW,SH=GameGlobal.SH;
